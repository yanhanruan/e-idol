...
import { Calendar, Gamepad2, Heart, Info, Mail, MapPin, Phone, User } from 'lucide-react';
...
const ReservationForm = () => {
  ...
  const games = ['Apex Legends', 'Valorant', 'League of Legends', 'スプラトゥーン3', 'Dead by Daylight', 'モンスターハンター', 'その他'];
  const casts = ['山田 花子', '佐藤 美咲', '田中 あかり', '鈴木 さくら', '指名なし'];

  const plans: PlanOption[] = [
    { id: 'online', name: 'オンライン通話プラン', price: '1時間 3,000円〜', desc: 'Discord・LINE通話でゲームや雑談' },
    { id: 'offline', name: 'オフライン対面プラン', price: '2時間 8,000円〜', desc: 'カフェやゲームセンターで実際にお会いします' },
    { id: 'coaching', name: '技術指導プラン', price: '1時間 4,500円〜', desc: 'ゲームスキル向上のためのコーチング' },
    { id: 'date', name: '恋人気分プラン', price: '2時間 12,000円〜', desc: 'デート感覚で楽しむ特別なひととき' },
    { id: 'casual', name: '通常エンジョイプラン', price: '2時間 6,000円〜', desc: '気軽にゲームや会話を楽しむ' },
  ];
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-content-secondary p-8 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-xs text-slate-700 leading-relaxed">
              ...
              <span className="font-medium">info@e-idol.net</span> からのご案内メールが届きます。
              <br />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-600">フルネームでのご記入を推奨します。複数名の指定も可能です。</p>
          </div>
          ...
          <div className="space-y-3 bg-amber-50 border border-amber-200 rounded-lg p-5">
            ...
            <div className="text-xs text-slate-700 leading-relaxed space-y-2">
              <p className="font-medium text-red-600">
                【注意】デート中にお酒/タバコ/不健康・パパ活/店舗ですぐない・などのお誘が出た時点でデートは終了とさせていただきます。サイトに記載される禁止事項・利用規約をよくお読んでおいてください。
              </p>
          ...
          <button type="submit" className="w-full bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white font-medium py-3.5 rounded-lg transition shadow-md hover:shadow-lg">
            予約を送信する
          </button>
